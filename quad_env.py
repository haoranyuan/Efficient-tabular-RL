...
class Quadcopter():
    ...
    def __init__(self, quads, motorlimits, gravity=9.81, b=0.0245, rewardflag='default'):
        self.quads = quads
        self.g = gravity
        self.b = b
        ...
        self.terminal_states = self.quads[key]['terminal']

        if isfile('reward_'+rewardflag+'.csv'):
            print('loading reward function')
            self.rewardscheme = np.genfromtxt('reward_'+rewardflag+'.csv', delimiter=',')
    ...
    def one_step(self, action, dt=0.05):
        ...
        return s_, r, done, success, s_cont_, a_cont_
